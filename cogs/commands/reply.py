            ...
            try:
                ...
                if len(args) > 2:
                    ...
                    if receiver_id == user.id:
                        receiver_id = c.execute(f'SELECT anon_sender FROM threads WHERE thread_id={thread_id}').fetchone()[0]
                    receiver = discord.utils.get(self.bot.users, id=receiver_id)
                    message_id = c.execute(f'SELECT max(message_id) FROM messages WHERE thread_id={thread_id}').fetchone()[0]
